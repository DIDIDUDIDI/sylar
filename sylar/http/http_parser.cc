...
namespace sylar {
    namespace http {
        ...
        /*
            限制header和消息体的大小，超过限制大小的我们都默认是恶意请求
        */
        static sylar::ConfigVar<uint64_t>::ptr g_http_request_buffer_size = 
               sylar::Config::Lookup("http.request.buffer_size", (uint64_t)(4 * 1024ull), "http reuqest buffer size");

        static sylar::ConfigVar<uint64_t>::ptr g_http_request_max_body_size = 
               sylar::Config::Lookup("http.request.max_body_size", (uint64_t)(64 * 1024 * 1024ull), "http reuqest max body size");
